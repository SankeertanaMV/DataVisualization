...
public class DBHelper extends SQLiteOpenHelper {
    ...
    private String CREATE_Dump_TABLE = "CREATE TABLE " + TABLE_Dump + "("
            ...
            + COLUMN_BillEP2 + " REAL,"
            + COLUMN_BillEP3 + " REAL,"
            + COLUMN_BillFLAT + " REAL,"
            + COLUMN_UsageEP1 + " REAL,"
            + COLUMN_UsageEP2 + " REAL,"
            + COLUMN_UsageEP3 + " REAL,"
            +COLUMN_UsageFLAT + " REAL);";
    ...
    public long createDump(String mailId, Float billep1, Float billep2, Float billep3, Float billflat,  Float usageep1, Float usageep2, Float usageep3, Float usageflat){
    ...
    public void insertIntoDump(){
        ...
        createDump("keerthi.mvsb@gmail.com",15f,5f,5f,5f,5f,5f,5f,5f);
    ...
    public boolean insertFlatno(Registerdetails registerdetails) {
        boolean isFlatnoExist = false;
        ...
        if(checkFlatnoExists(registerdetails.getFlatno()))
        {
            isFlatnoExist = true;
        }
        else
        ...
        db.close();
        return isFlatnoExist;

    }

    private boolean checkFlatnoExists(String flatno) {
        SQLiteDatabase db = this.getReadableDatabase();
        String query = "Select " +
                flatno + " from " + TABLE_Registerdetails;
        Cursor cursor = db.query(TABLE_Registerdetails,
                new String[] { flatno },null,null,null,null,null);
    ...
    public boolean checkFlatno(String flatno) {
        // array of columns to fetch
        String[] columns = {
                COLUMN_RID
        };
        ...
        // selection criteria
        String selection = COLUMN_Flatno + " = ?";

        // selection argument
        String[] selectionArgs = {flatno};
